...
これは、[standard](https://github.com/standard/standard)な JavaScript のルールの概要です。

`standard`を学ぶための最も良い方法は、ただインストールしてあなたのコードで試してみることです。

## ルール

- **インデントには 2 個のスペースを使用する。**
...
- **ブラウザのグローバルオブジェクトは `/* global */`コメントで宣言する。**<br>
  ...
  明示的に`window`の関数またはプロパティを参照するのも良いですが、そのようなコードは`window`の代わりに`self`を使う Worker では動作しません。
...
- **setter が定義されている場合、getter を必ず含むこと。**
...
- **配列初期化には Array コンストラクタではなく配列リテラルを使用する。**
...
- **switch 文の case 節でフォールスルーを防ぐため`break`を使用する。**
...
- **return 文における代入は括弧で囲まれていなければならない。**
...
- **制限付きの名前(訳注: restricted-name. strict mode の対象である ES5 の予約語)を上書きして隠蔽するべきではない。**
...
- **return 文、throw 文、continue 文、break 文の後に到達不能なコードは禁止。**
...
今日使用されている一般的なコードミニファイアー(圧縮ツール)は AST ベースのミニフィケーションを使用しているので、セミコロンレスな JavaScript を問題なく扱うことができます（JavaScript ではセミコロンは不要であるため）。

##### _["An Open Letter to JavaScript Leaders Regarding Semicolons"][1]_ からの抜粋：
