/**
 * Clones several projects that are known to follow "JavaScript Standard Style"
 * and runs the `standard` style checker to verify that it passes without
 * warnings. This helps ensure we don't accidentally introduce new style rules
 * that cause previously "good" code to start failing with new warnings! (And if
 * we do, then that needs to be a MAJOR VERSION BUMP.)
 */
...
const argv =
    minimist(process.argv.slice(2),
             {boolean : [ 'disabled', 'offline', 'quick', 'quiet', 'fix' ]})

let testPackages =
    argv.quick ? standardPackages.test.slice(0, 20) : standardPackages.test

const disabledPackages = []
testPackages = testPackages.filter(pkg => {
  if (pkg.disable)
    disabledPackages.push(pkg)
    return !pkg.disable
})
...
test('test github repos that use `standard`', t => {
  ...
  fs.access(path.join(TMP, name), fs.R_OK | fs.W_OK, err => {
    ...
    function runStandard(cb) {
      const args =
          [ STANDARD, '--verbose' ] if (pkg.args) args.push(...pkg.args)
    ...
    function runStandardFix(cb) {
      const args = [ STANDARD, '--fix', '--verbose' ] if (pkg.args)
                       args.push(...pkg.args)
      spawn('node', args, {cwd : folder}, err => {
        ...
        else {t.pass(str)} runGitReset(cb)
      })
    }
  ...
  if (err)
    ...
    function spawn(command, args, opts, cb) {
      if (!opts.stdio)
        ...
        child.on('close', code => {
          if (code !== 0)
            return cb(new Error(`non-zero exit code: ${code}`))
            cb(null)
        })
        return child
    }
